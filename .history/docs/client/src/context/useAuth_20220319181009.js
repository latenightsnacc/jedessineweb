...
export const AuthProvider = ({ children }) => {
    ...
    const [corperslist, setCorpersList] = useState([]);
    ...
    //     })
    // 
    const corpersList = async () => {
        const result = await getCorpersList();
        if (result) {
            setCorpersList(result);
            console.log(corperslist);
        }
    }
    const getCorpersList = async () => {
        try{
            return Axios.post(`${ROOT_URL}/getcorpers`, user.cdsGroup);
        } catch (e) {
            console.log(e);
        }
    }
    ...
    const corperLogin = async () => {
        console.log(details);
        try{
            return Axios.post(`${ROOT_URL}/auth/signin`, {
                email:details.email,
    ...
    const corperLogout = async () => {
        try{
            return Axios.get(`${ROOT_URL}/auth/signout`);
        }catch(e){
            console.log(e);
        }
    ...
    return(
        <AuthContext.Provider value={{user, alert, loading, auth, corperslist, setAuth, login, logout}}>
            {children}
