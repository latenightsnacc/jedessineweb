...
const Attendance = () => {
    ...
    for (let i = 1; i < results.length; i++){
        ...
        switch (m) {
            ...
            if(month[0].mar[0].records.length > 0){
                dates = [...new Set(month[0].mar[0].records.map(x => x.date))];
                //unique = month[0].mar[0].records.map(x => x.date).filter((e, i) => month[0].mar[0].records.map(x => x.date).indexOf(e) === i);
            };
            if(dates){
                unique = dates;
                month[0].mar[0].dates.push(...unique);
            }
            break;
            case 'April': 
