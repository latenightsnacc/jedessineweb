...
app.post("/recordattendance", async (req, res) => {
    console.log(req.body);
    const records = Object.keys(req.body);
     ...
     try{
         if(!req.body ){
            ...
            for(let i = 0; i < records.length; i++){
                const date = req.body[i].date_recorded;
                const month = req.body[i].month_recorded;
                const year = req.body[i].year_recorded;
                const type = req.body[i].attend_for;
                const status = req.body[i].attendance_status;
                const comment = req.body[i].comt;
                const corperId = req.body[i].id;
                pool.query("SELECT * FROM attendances WHERE date = ? AND type = ? AND corperId = ?", [date,type,corperId], (err, result) => {
                    if(err){
                        console.log(err);
                    } else if(result.length > 0){
                        pool.query("UPDATE attendances SET status = ? AND comment = ? WHERE date = ? AND type = ? AND corperId = ? ", [status, comment,date,type,corperId], (err, result) => {
                            if(err){
                                console.log(err);
                            } else {
                        ...
                        const recordAttendance = db.recordAttendance(date,month,year,type,status,comment,corperId).then(response => {
                            return response;
                        });
                        if(recordAttendance){
                            ...
                            return res.status(500).send({
                                message: "Error occured while saving attendance! "
                            });
                        }
                    }
                })
...
app.get("/attendance", async (req, res) => {
    pool.query("SELECT * FROM attendances", (err, results) => {
        if(err){
            console.log(err);
        } else {
            res.send(results);
