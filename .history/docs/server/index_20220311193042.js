...
app.post("/api/auth/signup",vupload.single('profilePic'), async (req, res) => {
    try{
        if(!req.body || !req.file){
            ...
            const profilePic= "http://localhost:3000/images/"+req.file.filename;
            const password= bcrypt.hashSync(req.body.statecode, salt);
            const status= req.body.status;
            const roles= req.body.roles;
            const state= req.body.state;

            const insertCorper =db.insertCorper(firstname,surname,batch,statecode,cdsGroup,lga,ppa,email,phoneNo,profilePic,password,status,roles,state);
