            ...
            for(let i = 0; i < records.length; i++){
                ...
                pool.query("SELECT * FROM attendances WHERE date = ? AND type = ? AND corperId = ?", [date,type,corperId], (err, result) => {
                    if(err){
                        ...
                        async pool.query("UPDATE attendances SET (status, comment) VALUES(?,?) WHERE date = ? AND type = ? AND corperId = ? ", [status, comment,date,type,corperId], (err, result) => {
                            if(err){
                                console.log(err);
                            } else {
                                res.status(200).send({
                        ...
                        const recordAttendance = await db.recordAttendance(date,month,year,type,status,comment,corperId);
                        if(recordAttendance){
