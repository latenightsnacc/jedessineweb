...
db.recordAttendance = async (date, month, year, type, status, comment, corperId) => {
    return new Promise((resolve, reject) => {
        async pool.query('INSERT INTO attendances (date, month, year, type, status, comment, corperId) VALUES(?,?,?,?,?,?,?)', [date, month, year, type, status, comment, corperId], (error, result) => {
            if(error){
                return reject(error);
            }
