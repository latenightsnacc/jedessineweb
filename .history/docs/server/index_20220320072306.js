...
app.post("/api/auth/signin",  async (req,res) => {
    try{
        ...
        const corper = db.getCorperByEmail(email);
...
app.get("/getcorpers", async (req, res) => {
    const cdsGroup = req.body.cdsGroup;
    const status = req.body.status;
    if(req.session.user){
        const corpers = await db.getCorpersByCdsGroup(cdsGroup, status);
        if (corpers){
            res.send(corpers);
        }
    }
})
app.post("/getcorpers",  async (req,res) => {
    try{
        const cdsGroup = req.body.cdsGroup;
        const status = req.body.status;
        const corpers = db.getCorpersByCDS(cdsGroup, status);

        if(!corpers){
            return res.send({
                message: "Your members have not created their profiles."
            })
        } else res.status(200).send(corper)
        console.log("Logged in");
    }catch(e){
        console.log(e);
    }
